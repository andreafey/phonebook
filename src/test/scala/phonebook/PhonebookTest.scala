...
// Also See openOutput for performing several writes with a single connection
    ...
    def cleanup = {
        // TODO copy existing defaultpb and overwrite instead
        rm(new File(defaultpbname))
//        val defaults = List("Barack Obama")
//        val defaultNums = List("212 555 1212")
//        val pbdata = (for {
//                (na, nu) <- defaults.zip(defaultNums)
//            } yield "'%s' '%s'".format(na, nu)).mkString("\n")
//        val file = new File(defaultpbname)
//        val writer = new PrintWriter(pb)
//        writer.print(pbdata)
//        writer.close
        rm(new File(mockdir))
	...
	test ("add should add someone if they're not in the pb") {
        ...
        out.toString.contains("entry added") should be (true)
	...
	test("entries should return the entries in an existing file") {
	    val entries = Phonebook.fromFile(new File(pb)).entries
...
//	}
	test("pb -> add should immediately make item available in trie for lookup") {
	    val name = "Big Bird"
        val number = "1 800 SESAMEST"
        val phonebook = Phonebook.fromFile(new File(pb))
	...
	test ("class -> lookup should find person in pb") {
        val phonebook = Phonebook.fromFile(new File(pb))
        val name = "Bob Orangutan"
        val matching = phonebook.lookup(name)
        matching.isEmpty should be (false)
        matching.head.name should be (name)
...
//	}
	test ("class -> reverse-lookup should find person in pb") {
        ...
        it.head.name should be ("Bob Orangutan")
	}
